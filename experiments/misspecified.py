...
def main():
    ...
    add("to look for a signal that is there. The zero-bits result is exact")
    add("given the tables and worth nothing if the tables are wrong.\n")
    add("> **Amended after W2-8.** What I wrote here first was that the failure")
    add("> is silent -- that nothing in the agent's own diagnostics can detect")
    add("> it, because the invariance is an assumption about the likelihood")
    add("> tables rather than an observation, and the agent never observes its")
    add("> own tables being wrong. I also wrote that the only way to find out")
    add("> was to measure the likelihoods against real labelled findings, which")
    add("> this project has never been able to do. Both claims are false, and")
    add("> `model_uncertainty.py` is what falsified them. A Bayes factor")
    add("> between H0 (the rows are identical) and H1 (they differ), fed only")
    add("> the labels the agent's own biased discovery process surfaces, flags")
    add("> the broken invariance in 100% of runs at δ ≥ 0.10 -- the same δ")
    add("> values priced above at +1.47 and +1.99 minutes. No real labels, no")
    add("> oracle, no clean feedback. See `results/model-uncertainty.md`.\n")
    add("What survives the amendment is the limitation underneath it. The agent")
    add("can now notice the assumption is wrong; it still cannot repair it,")
    add("because relaxing the invariance means estimating two separate rows and")
    add("there are not enough labels to do that well. Detection and repair are")
    add("different problems and only the first one is solved.\n")
    ...
    add(f"| Free features carry zero bits about live vs revoked | **fails "
        "whenever the world says otherwise** -- detectable from the agent's "
        "own feedback at δ ≥ 0.10 (W2-8), not repairable |")
    add("")

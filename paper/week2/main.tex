...
On the domain, the comparative evaluation of detection
tools~\cite{basak2023comparative} establishes that detection is largely solved
and that the surviving problem is what to do with a detection; Alahmadi et
al.~\cite{alahmadi2022falsepositives} quantify the false-positive burden that
makes triage cost-dominated. GitGuardian's
...
\paragraph{The cost matrix.} Every number in this paper is denominated in
engineer-minutes, and this is the whole of it. The four terminal actions against
the five states:
...
\begin{tabular}{@{}lrrrrr@{}}
\toprule
& \texttt{live} & \texttt{revoked} & \texttt{fake} & \texttt{zero\_scope} & \texttt{other} \\
\midrule
\textsc{dismiss} & 244.5 & 2 & 2 & 5 & 63.4 \\
\textsc{revoke now} & 332 & 2 & 5 & 5 & 86.0 \\
\textsc{rotate safely} & 112 & 30 & 20 & 30 & 48.0 \\
\textsc{escalate} & 142 & 32 & 32 & 35 & 78.0 \\
\bottomrule
\end{tabular}
\end{center}

These are my estimates, not measurements, and Section~\ref{sec:limits} says so at
length. Three of them are derived rather than chosen. \textsc{dismiss} on a
\texttt{live} key is $P(\text{exploitation}) \times \text{breach}$, which
Section~\ref{sec:failure} is about. \textsc{escalate} is thirty minutes of human
attention plus whatever the person then does. The \texttt{other} column is the
mean over the four known states, which is what makes it move whenever the breach
cost moves---and forgetting to recompute it after that cost changed cost me an
afternoon.
...
those two things is a dataset nobody can build.

Labelled secret data does exist, and it labels the wrong thing. The public
corpora label real credential against false positive, which is the axis my free
features already resolve. My axis is live against revoked, and that one cannot
be labelled after the fact: establishing whether a credential leaked two years
ago still authenticated at the time would have required trying it at the time,
which is the single act this design forbids. The gap is therefore structural
rather than a matter of effort. It is not that nobody has done the work; it is
that the measurement destroys the thing being measured, and any corpus that did
carry the label would be evidence that somebody authenticated with credentials
they found in other people's repositories.
